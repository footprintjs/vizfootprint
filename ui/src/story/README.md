...
And the whole thing as one file has a door of its own again. [`page/`](./page/README.md) — `vizfootprint-ui/story/page` — is a BUILD: a host bundles its definition with the engine, the charts and the stage, and the log, the bookmarks, the pictures and the data ride in a script block the page reads on load. It opens from `file://` with no server, and every beat carries a door that forks a path of the reader's own so their acts never land on the author's.
