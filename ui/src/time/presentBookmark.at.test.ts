...
const S = mapPollState({
  ...
  head: 'f',
  bookmarks: [
    { label: 'named at a', commitId: 'b1', at: 'a', ts: 2, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' },
    { label: 'named at t', commitId: 'b2', at: 't', ts: 4, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' },
  ],
...
describe('bookmarkTarget', () => {
  it('is the named position when known, the bookmark itself on an older wire, and null when neither is known', () => {
    expect(bookmarkTarget({ label: 'x', commitId: 'b', at: 'a', ts: 0, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' })).toBe('a');
    expect(bookmarkTarget({ label: 'x', commitId: 'b', at: null, ts: 0, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' })).toBe('b');
    expect(bookmarkTarget({ label: 'x', commitId: 'b', ts: 0, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' })).toBe('b');
    expect(bookmarkTarget({ label: 'x', commitId: null, at: null, ts: 0, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' })).toBeNull();
  });
});

describe('bookmarkRefTarget — a note links a bookmark by ID, and lands on the same commit the slideshow does', () => {
  const BOOKMARKS = [
    { id: 'b1', label: 'the spike week', commitId: 'c7', at: 'c7', ts: 1, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' },
    { label: 'an older bookmark', commitId: 'c3', ts: 2, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' }, // a wire that predates bookmark ids
  ];
  ...
  it('an id wins over a same-named label, and an unknown ref resolves to nothing (a click that does nothing, never a wrong seek)', () => {
    expect(bookmarkRefTarget([{ id: 'b2', label: 'b1', commitId: 'cA', ts: 1, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' }, { id: 'b1', label: 'x', commitId: 'cB', ts: 2, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' }], 'b1')).toBe('cB');
    expect(bookmarkRefTarget(BOOKMARKS, 'b9')).toBeNull();
    expect(bookmarkRefTarget([{ id: 'b1', label: 'floating', commitId: null, ts: 1, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' }], 'b1')).toBeNull();
  });
  ...
  it('a legacy `bookmark:` commit lands on the moment NAMED, never on the act of naming — the drift a second resolver produced', () => {
    ...
    const legacy = [{ id: 'b1', label: 'named at a', commitId: 'b1c', at: 'a', ts: 2, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' }];
...
describe('a fork below a named commit keeps the bookmark', () => {
  ...
  it('two bookmarks naming the SAME position keep their naming order', () => {
    const twice = [...S.bookmarks, { label: 'named at a, again', commitId: 'b3', at: 'a', ts: 9, by: 'user' as const, madeAt: '2026-01-01T00:00:00.000Z' }];
