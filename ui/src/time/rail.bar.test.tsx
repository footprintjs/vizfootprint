...
describe('the rail in the bar', () => {
  it('explore: the two step buttons sit in one group before the rail; the rail carries the tick variable and every commit as a dot', () => {
    const { container } = render(<TimeTravelBar compact commits={MANY} cursor="c39" head="c39" bookmarks={BOOKMARKS} />);
  ...
  it('each bar says where it stands: the cursor, the head, and everything AHEAD of the cursor — and only a bookmark carries a flag', () => {
    const { container } = render(<TimeTravelBar compact commits={MANY} cursor="c10" head="c39" bookmarks={BOOKMARKS} viewingPast />);
    ...
    // the flag slot: a bookmark shows ⚑, every other bar keeps the same empty box, so no bar sits lower than its neighbour
  ...
  it('a cursor that is on no bar of this lineage leaves nothing "ahead" — and the head is still marked', () => {
    const { container } = render(<TimeTravelBar compact commits={MANY} cursor={null} head="c39" bookmarks={BOOKMARKS} />);
    const rail = container.querySelector('[data-vzf="timeline"]')!;
  ...
  it('present: bookmarks on a rail of their own, the step group first, and Play only when the host brought it', () => {
    const onPlay = vi.fn();
    const { container, rerender } = render(<TimeTravelBar compact mode="present" commits={MANY} cursor="c39" head="c39" bookmarks={BOOKMARKS} onPlay={onPlay} />);
    ...
    const rail = row.querySelector('[data-vzf="bookmark-rail"]') as HTMLElement;
    expect(rail.querySelectorAll('.vzf-tl-dot')).toHaveLength(2);
    expect(rail.style.getPropertyValue('--vzf-tick')).toMatch(/^\d+px$/);
    fireEvent.click(row.querySelector('[data-vzf="play"]')!);
    expect(onPlay).toHaveBeenCalledTimes(1);
    rerender(<TimeTravelBar compact mode="present" commits={MANY} cursor="c39" head="c39" bookmarks={BOOKMARKS} />);
...
describe('the rail under pressure', () => {
  it('a narrow rail turns dense: ids hide, flags and dots stay; a resize observer re-measures; the past is a mark in the compact strip and a banner in the full one', () => {
    ...
    try {
      const { container, unmount } = render(<TimeTravelBar compact commits={MANY} cursor="c39" head="c39" bookmarks={BOOKMARKS} viewingPast />);
      ...
      expect(rail.querySelectorAll('.vzf-tl-dot')).toHaveLength(40); // never a tick dropped
      expect(rail.querySelectorAll('.vzf-tl-flag')).toHaveLength(2); // the bookmarks keep their flags
      ...
      const full = render(<TimeTravelBar commits={MANY} cursor="c39" head="c39" bookmarks={BOOKMARKS} viewingPast />).container;
      ...
      // a rail that mounts LATER (present mode with no bookmarks, then bookmarks named) is measured when it appears
      const late = render(<TimeTravelBar compact mode="present" commits={MANY} cursor="c39" head="c39" bookmarks={[]} />);
      expect(late.container.querySelector('[data-vzf="bookmark-rail"]')).toBeNull();
      late.rerender(<TimeTravelBar compact mode="present" commits={MANY} cursor="c39" head="c39" bookmarks={manyBookmarks} />);
      expect(late.container.querySelector('[data-vzf="bookmark-rail"]')!.getAttribute('data-dense')).toBe('true');
      // the past mark carries its sentence for assistive tech, the glyph is decoration
      const mark = render(<TimeTravelBar compact commits={MANY} cursor="c39" head="c39" bookmarks={BOOKMARKS} viewingPast />).container.querySelector('[data-vzf="past-mark"]')!;
