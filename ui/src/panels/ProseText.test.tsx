...
describe('pieces', () => {
  it('cuts the text at the refs in order, skipping overlaps and out-of-range spans', () => {
    ...
    expect(pieces(text, [{ span: [16, 29], bookmark: 'b1' }, { span: [0, 8], commit: 'c1' }, { span: [4, 10], commit: 'x' }, { span: [20, 99], commit: 'y' }]).map((p) => [p.text, p.ref?.commit ?? p.ref?.bookmark ?? null, p.from, p.to])).toEqual([
      ['Oklahoma', 'c1', 0, 8],
      [' leads; ', null, 8, 16],
      ['Texas follows', 'b1', 16, 29],
      ['.', null, 29, 30],
    ]);
...
describe('ProseText', () => {
  it('renders anchors that describe and go to their target; a bookmark goes to onBookmark by its bookmark ID; a label overrides the words', () => {
    const onSeek = vi.fn();
    const onBookmark = vi.fn();
    render(
      <ProseText
        text="Oklahoma leads; Texas follows."
        refs={[{ span: [0, 8], commit: 'c1' }, { span: [16, 21], bookmark: 'b1', label: 'week 1' }, { span: [22, 29], commit: 'c2', label: 'see the run' }]}
    ...
    fireEvent.click(screen.getByRole('button', { name: 'go to bookmark "week 1"' }));
    expect(onBookmark).toHaveBeenCalledWith('b1'); // the ID travels, never the name
  ...
  it('a ref with no target still renders, as an anchor that goes nowhere', () => {
    ...
    fireEvent.click(screen.getByRole('button', { name: 'go to bookmark ""' }));
    expect(onSeek).not.toHaveBeenCalled();
  });
  it('without handlers the anchors are inert', () => {
    render(<ProseText text="ab" refs={[{ span: [0, 1], commit: 'c' }, { span: [1, 2], bookmark: 'b' }]} className="mine" />);
    fireEvent.click(screen.getByRole('button', { name: 'go to commit #c' }));
    fireEvent.click(screen.getByRole('button', { name: 'go to bookmark "b"' }));
