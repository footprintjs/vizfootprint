...
1. Call whats_here FIRST. It reports the declared views and their ids, EACH view's current channel->field visual encodings, the columns you may name (columns, keyed by table — every view reads columns[defaultTable]; branch-scoped, names+types only) and per view accepts: which of those columns actually FIT each channel right now, the active DATA-space selections, the declared analyses and whether each is ready to run, the online-FDR ledger, how many requests have gone unmet (gaps), the NAMED PATHS of the history (which path you're on, every path with its tip), and layouts — the cockpit's CURRENT on-screen arrangement (e.g. layouts.dashboard.preset, layouts.dashboard.focus). Orient before you act — columns[defaultTable] is how you know which fields actually exist before you name one, and a view's accepts is how you know which of them may go on a given channel, and layouts is how you know what is actually on screen before you change it.
...
2b. Change what an axis SHOWS with dispatch verb 'reencode': give viewId, channel (e.g. 'x', 'y', 'color' on the scatter; 'category' on the bar), and field — field must be one of the names in whats_here's accepts for that view's channel (a column in columns[defaultTable] that FITS it), and the channel must be one whats_here's view.encodings already has an entry for. An invalid channel or a column that doesn't exist comes back as a typed gap, not a guess.
