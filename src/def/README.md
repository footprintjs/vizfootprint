...
`types.ts` the schema · `validate.ts` the firewall + the parse door · `builtinAnalyses.ts` an analysis as data · `register.ts` the one registry boundary · `buildDashboard.ts` the build · `series.ts` the long-form series contract · `recordIds.ts` the id counters
