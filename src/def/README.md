...
13. **A BAND IS A RANGE TOO, AND A DECLARED GESTURE THE RENDERER WILL NOT DRAW IS REFUSED AT THE DOOR.** `capabilities[i].encodings` is a declaration of a view's VOICE, and this library judges declarations: a view that says it emits an `interval` must be a view a reader can actually drag. The defect that produced this law was measured on a real page — a line chart of 185 residues declared `{ canProbe: true, encodings: ['interval'] }`, its x was a **band** (two chains share one residue-number axis, so a slot holds a residue of each), the chart drew **no brush element of any kind**, and a drag left every count unchanged: 185 dots before, 185 after. The page promised a drag, a caption told the reader to drag, and nothing happened. That is capability law 1 broken at the declaration.
