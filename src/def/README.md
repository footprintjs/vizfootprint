...
### What a column IS — and who may say it

`data[<table>].columns` is what the caller STATES about the table's own columns, one entry per field: a **role** (`identifier | dimension | measure`), a **scale** (`discrete | continuous`), a **label**, a **unit**, and a **type** where the provider's reading is not the truth (`ColumnDecl`, `../encoding/types.ts`). Declared or absent, never guessed — a rule that needs a role simply does not match a column that never declared one. The one exception is the SCALE, which falls back to the type (`number`/`date` → continuous, `string`/`boolean` → discrete) when nothing said otherwise, and the table's declared state columns, whose role `absence` is derived from `absence` rather than stated.

The four words minus the type are `ColumnMeaning` (`../data/types.ts`), and they are one vocabulary with **two speakers**. The other is an ACT: an analysis on the `columns` channel says what it lands in the same four words (`OutputColumn`, [`../analysis/README.md`](../analysis/README.md), "The `columns` channel: an act declares what it LANDS"), which is how a landed RANK says it is a place rather than a magnitude without the definition having to say it on the act's behalf.

**One column, one owner, and the act owns the one it lands.** When an act declares any of the four words, its declaration is the whole declaration for that column and this map is not read for it. When the act says nothing — every act written before an act could speak — this map applies exactly as it always did. A def entry for a column a speaking act lands is not merged and not silently overridden: it is refused by name where the act lands, and the repair is to delete one of the two. The sentence is in the analysis README beside the rule.

What a MINTED table's columns can declare is still nothing but a type (law 5a below, and `mintedTables`): an aggregate computes the names and types of the table it will mint, and carries no role, no scale, no label and no unit. That is unchanged here — the vocabulary now exists in a place a minted column could one day borrow it from, and no second path was built to do so.
...
13. **A BAND IS A RANGE TOO, AND A DECLARED GESTURE THE RENDERER WILL NOT DRAW IS REFUSED AT THE DOOR.** `capabilities[i].encodings` is a declaration of a view's VOICE, and this library judges declarations: a view that says it emits an `interval` must be a view a reader can actually drag. The shape that produced this law was measured on a real page — a line chart declared `{ canProbe: true, encodings: ['interval'] }`, its x was a **band**, the chart drew **no brush element of any kind**, and a drag left every count unchanged. The page promised a drag, a caption told the reader to drag, and nothing happened. That is capability law 1 broken at the declaration.

    **The CAUSE recorded here was wrong, and it was recorded twice.** The chart on that page whose drag did nothing was not standing on a band: its x held residue **NUMBERS**, and the line positioned every run through `Date.parse`, so it emitted date-shaped strings a numeric column can never answer (162 marks before the drag and 162 after, the refusal ledger climbing once per drag). This law and its door are still right and still earn their place — a band's drag IS a run of slots, and a band line draws one now — but the evidence quoted for them belonged to another defect, fixed in `ui/README.md`, "A run over numbers emits numbers". Left standing rather than quietly deleted: a wrong recorded cause is worse than none, because the next reader builds on it.

    Both halves landed together, because either alone leaves the page lying. **The picture's half:** a band line now brushes its band — a drag selects the slots whose POINTS it crosses and lands the `match` a band already speaks, which is the very clause a bar's own drag over one band lands (`vizfootprint-ui/primitives/scales.ts` · `slotsCovered`; `ui/README.md`, "VizLine"). **The door's half**, one refusal, one predicate (`drawsIntervalBrush`) and one sentence with two arms (`intervalGestureRefusal`), said here with an address and by the frame that has to draw it exactly as it stands (`vizfootprint-ui/contract/renderers.tsx` · `intervalBrushRefusal`) — the law 9 and 12 arrangement, so a def this door accepts is never a gesture the renderer refuses:
    ```
    ...
    ```
    Two arms because the two mistakes have two different repairs (the `refuseOwnScale` precedent): a mark whose x is a band DOES have a drag and it lands a match, so the repair is to declare that; a mark that draws no brush at all has no drag to declare, so the repair is to drop the key. The first arm names the COLUMN, because a field is a band because of its DATA and an author has to be able to see which column made it one. The three marks that draw a horizontal interval brush are a **scatter** (a point, under both its names), a **line** and a **histogram**; a bar's drag was always a match, a box plot has no drag, a heatmap's gesture is the compound cell, a map has regions, a table rows, a network a walk. The refusal's ADDRESS is the `capabilities[]` key and not the encoding surface, because the declaration that is wrong is the capability's — the marks and the bindings are a legitimate picture. It is judged per LAYER as well as per view: the frame's capability is the layers' capability.

    **What this door CANNOT refuse: whether the x column is a band.** A field is a band because its VALUES are categorical, and a declared table's types are the PROVIDER's — so where the def types nothing the column answers `unknown`, `frameScaleOf` answers nothing for it, and this door declines. That is law 11b's own discipline (`refused on evidence, never on ignorance`, the `unit` precedent), and it is why this law is not a complete fence. The rest lives where the evidence is, in two places. The **CHART**, which knows what it actually drew: on a band it delivers the gesture it can — the drag lands the slots it crossed — so a reader gets the gesture rather than a silence, and a drag that crossed no point at all says so out loud (`noSlotsCoveredNote`, through the library's one polite live region) instead of emitting an empty keep-list. And the **session's probe guard**, which meets the emitted kind against the declared voice and says *`view "surface" does not encode a match selection`*. So a reader always ends with either the gesture or a sentence, and never with silence.

    **AND THE LAYER THIS DOOR CANNOT REACH AT ALL: the VALUE.** A declaration carries no values, so `drawsIntervalBrush` can say *a band has no between for an interval* and still pass a renderer that draws a brush over a NUMBER and emits date-shaped strings for it — the declared KIND delivered, and nothing the column can answer. That is the same lie one layer in, and it is judged where the clause is: `intervalAddresses` / `unaddressableIntervalRefusal` (`../encoding/frame.ts`, beside this law's own predicate), asked by **the session's own probe door** — `unaddressableClause` / `unaddressableValueRefusal`, the fan-out over all three single-column kinds, filed under the gap code `unaddressable-value`. The rule is the interval evaluator's own no-cross-type-coercion law read backwards — a quantitative axis is addressed by numeric bounds, a categorical one by strings, a temporal one by either spelling of a date — and it declines to judge anything it has no evidence about (an unfoldable type, a cleared interval, an open side):
    ```
    ...
    ```
    **THAT FENCE MOVED TO THE DOOR, and it now covers the POINT and the MATCH too.** The hole recorded here — *the session lands such a clause, matches no row with it and files no gap* — is closed. Why it had to be the door rather than the conformance kit was measured by the defect that followed: a band drawn over a column of numbers emitted a `match` of its slots' SPELLINGS, the door took it, the record gained a commit (4 → 5, the refusal ledger unchanged at 3) and 185 marks in force became 0. **A landed clause that kept nothing is worse than a refusal**, and the kit could only ever see the states a plan built. So the door judges every clause anybody dispatches, and the kit's own value-level arm is gone rather than doubled — the door runs first, such a gesture lands no commit, and the kit's `commit-lands` step quotes the door's sentence verbatim (`vizfootprint-ui/contract/README.md`).

    **WHAT THIS FENCE HONESTLY CANNOT CATCH, named rather than implied.** A band exists because its column folded CATEGORICAL, and the categorical fold NAMES every cell it is given (`frameDomains`) — so a column reported as text may honestly hold numbers, and refusing a value against that declaration would refuse clauses that keep rows. A band over a `string` or `boolean` column is therefore **not judged** at this door; there the only fence is the chart tier itself (`slotValues`, the one owner of what a slot's name stands for) and the tests that pin it. The door catches a band drawn over a column the session types as a NUMBER or a DATE — the shape a host builds when it wants slots over a numeric column, and the shape the first-party conformance suite now builds as its fourth line plan.

    A declaration that does not name `interval` is untouched, byte for byte — no new key, no new sentence, and a view with no capability at all keeps the assumed voice it always had.

14. **WHAT THE QUANTITY CAN BE — a numeric extent CAN be declared, and it is a fact about the QUANTITY rather than about the rows.** `bounds?: readonly [number, number]` says the extent this axis is read on: `[-180, 180]` for a backbone torsion angle, `[0, 100]` for a percentage, `[0, 1]` for a probability, `[-1, 1]` for a correlation. It rides on the SHARED mode and is one of the axis keys law 7 keeps on a layerless view. Default: the fold's own union over the rows, exactly as before.

    **Why it is declarable, when law 7 says a frame declares a fold and never numbers.** Those are two different claims and this is the opposite one, not an exception to it. `domain: 'union'` is a word about how LAYERS combine their extents, and the numbers it produces are folded so that an axis can never disagree with the data under it. `bounds` is what the quantity can be *by definition* — a torsion angle is −180…180 whatever residues are in the table — which no fold over rows can know and no fold may overwrite. **The axis says what the quantity CAN be; the marks say what this entry HAPPENS to be**, and conflating the two is the defect that produced this law. It was measured on a real page: a Ramachandran plot whose axes were folded from the residues present drew one at 107° hard against the right edge, where it reads as the edge of torsion space when it is nowhere near it — and the consumer shipped the range as a host PROP, on no commit, because there was nowhere to declare it. A prop the record never sees is a picture whose provenance cannot name its own furniture (law 12's argument, again).

    **Why not `domain`, and why not `range`.** `domain` is taken, for the opposite idea, above. `range` is taken TWICE already and one of them points the other way: the agent tool surface spells a FILTER's interval `range` (`../agent/vizAsTools.ts` — `range: null` clears it), which narrows the ROWS, and `linearScale` spells the PIXEL pair `range` (`vizfootprint-ui/primitives/scales.ts`), which is d3's meaning. A third meaning of one word, pointing away from the first, is how a vocabulary stops being readable. `extent` and `span` are the ui primitives' words for the DATA's own pair, which is precisely what this is not. `bounds` is plain, is a plural that says it is a pair, and was free.

    Four refusals — the shape, then the three things the columns and the curve make judgeable:
    ```
    encodings[0].frame.y.bounds, if present, must be a pair of finite numbers with the low one first — what the QUANTITY can be, e.g. [-180, 180] for a torsion angle
    encodings[0].frame.y.bounds needs a number — layer "residues" binds y to "chain", a string
    ...
    encodings[0].frame.color: unknown key "bounds" on an independent channel
    ```
    A REVERSED pair is a typo and a FLAT one is not an extent, so neither is read as "what the quantity can be". The column half is the logarithm's own discipline, read through the same declared facets (`refused on evidence, never on ignorance` — an undeclared column type refuses nothing): bounds ARE numbers, so on a date or a category they would be read as nothing at all, and a declared fact that reaches no picture is the whole defect this law closes. The logarithm half is refused because the chart's `scaleFor` LIFTS a non-positive bound to keep its scale finite (`logDomain`, its one owner), so a declared `[0, 100]` on a log axis would draw an axis nobody asked for. And an INDEPENDENT channel is refused by name with the other keys it has no use for: each layer keeps its own scale there, so there is no one axis for one claim.

    **A declared extent never hides a value.** `ResolvedChannel` carries `bounds` BESIDE `domain` rather than replacing it, so a reader can see a table covering a third of its own axis; the CHART prefers the declared pair when it draws (`spanOf`, `vizfootprint-ui/contract/renderers.tsx`). A cell outside it is still placed at its true position — nothing is dropped, the band axis's law — and because a mark past the plot edge is invisible it is **counted and said**, in the picture and in the accessible name, in the register the logarithm's exclusions already use (`outsideNotes`, `vizfootprint-ui/primitives/scales.ts`): *`x has 3 values outside its declared bounds [0, 100] — bounds say what the quantity CAN be, so either they are wrong or this data is`*. A value outside means either the data is wrong or the claim is, and either way the reader is the one person who can tell which.

    What this law does NOT do: fold anything. A channel with no foldable cell at all still gets no entry (the existing "nothing folds" arm), so declared bounds over an empty table reach no picture — an axis drawn over no data is a different question and a different packet.
...
The shape sentences, for completeness: `encodings[i].layers, if present, must be an array of { layerId, table, chartKind, channels }` · `encodings[i].layers[j] must be an object { layerId, table, chartKind, channels, initial?, label? }` · `encodings[i].layers[j]: unknown key "x"` · `…layerId must be a non-empty string` · `…chartKind must be a non-empty string` · `…channels must be a non-empty array of non-empty strings` · `…initial, if present, must be an object mapping channel -> field (strings)` · `…label, if present, must be a string`. A table refused on its own line is not refused again through a layer, and a layer on it is not judged at the build door. The frame's shape sentences: `encodings[i].frame, if present, must be an object mapping channel -> { mode: "shared" | "independent" }` · `encodings[i].frame.<channel> must be an object { mode: "shared" | "independent", domain?, basis?, guide?, zero?, transform?, zeroGuide?, bounds? }` — on a LAYERLESS view the same channel is named against the axis shape instead, `{ transform?: "linear" | "log", domain?, basis?, guide?, zero?, zeroGuide?, bounds? }`, and an unknown key there is refused "on an axis" · `…basis, if present, must be "table" or "rows"` · `…guide, if present, must be "merged" or "per-layer"` · `…zero, if present, must be a boolean` · `…transform, if present, must be "linear" or "log"` · `…zeroGuide, if present, must be a boolean` · `…bounds, if present, must be a pair of finite numbers with the low one first — what the QUANTITY can be, e.g. [-180, 180] for a torsion angle` · `encodings[i].frame.<channel>: unknown key "x" on a shared channel`. Not in this version: per-layer opacity/visibility dials, annotation layers, re-encoding one layer of a frame, a map frame with an inset, an implicit crossfilter between sibling layers (only a declared link routes between them), a symlog or a power transform, and a per-layer transform on a shared channel (one resolution per channel) — each its own packet.

15. **A VIEW THAT IS OUTSIDE THE GRAMMAR MAY SAY WHY, AND THE LIBRARY NOW SAYS THAT IT IS.** `capabilities[i].silentBecause?: string` — the def's own words for why no clause can ever be about a view — declarable only beside `canProbe: false`, which is the whole of what it explains.

    **The measured defect.** A desk had a pane whose rows are not in the data space at all (a 3D viewer over a structure file), declared `{ canProbe: false }`. When every pane around it narrowed and it did not, the only sentence it could say was the crossfilter's — *"the selection elsewhere cannot be judged here"* — which is what a pane says when a clause **set out and could not be judged**, and reads exactly like a fault. The truth is the opposite: no clause can ever be **about** that view. The host rebuilt that distinction from two booleans (`canProbe: false` and an empty `selectionKinds`) and wrote its own words for it, which is omit-never-deny broken in the plainest way — the library knew the fact and did not say it.

    So the session serves `ViewInfo.silent` (`ui` · `ViewView.silent`): **the TYPED reason always, the declaration's OWN words only when it wrote any.** The derivation is the library's law — what *outside the grammar* means is not a host's to decide, and the two booleans a host was reading are one fact (`canProbe: false` ⇒ `voiceOf` returns no kinds at all). The SENTENCE is the author's: the library carries words, it does not author them for a reader's screen, the `label` / `chartKind` precedent. A renderer can therefore tell *no clause can reach me* from *a clause reached me and I could not judge it* without inference.

    ```ts
    capabilities: [{ viewId: 'viewer', canProbe: false, silentBecause: 'the camera is the viewer’s own — there is nothing here a clause could name' }]
    // → views[].silent === { reason: 'declared', words: 'the camera is the viewer’s own — …' }
    ```

    Two refusals, both at the `capabilities[]` key that fixes them: blank words would put an empty explanation on a screen, and a reason declared beside `canProbe: true` is a contradiction the author should hear about at build rather than a sentence nothing would ever serve.
    ```
    capabilities[1].silentBecause, if present, must be a non-empty string
    ...
    ```
    `reason` is a discriminant and not a boolean so a second cause of silence can arrive without a second key. The mount-time twin is `AdapterCapabilities.silentBecause`, for the same reason the `encodings` twin exists.
...
**A body large enough to wait for.** A declared resource may be 169 MB (measured: a protein family's full alignment, still arriving after 60 s), so both doors take a way to be TOLD how it is going: `buildDashboardAsync(def, { onResourceProgress })` while it builds, and `dashboard.refresh(names?, { onResourceProgress, signal })` afterwards. A report is a report — `{ resource, bytes, total? }`, transient, on no commit, and nothing computes from it — while the **state** it puts on `overview().resources` is a fact (`state: 'arriving'`, absent when the bytes are simply held). And the half that matters: **a resource is not landed until it is whole.** A read that is cancelled, cut off or capped lands no version, is refused by name, and leaves yesterday's bytes exactly where they are — a half-arrived alignment is the first N sequences in file order, not a smaller alignment. A host that passes neither option gets the read this library always made, byte for byte. The law, the gzip trap it is built around and the reason for the second half are in [`../source/README.md`](../source/README.md), "Bytes may arrive progressively". A SYNCHRONOUS dashboard takes the same options and honestly reports nothing: an inline payload is the def's own text, and it never arrives over anything.
...
| `growing.ts` | the SECOND landing judged against the DECLARATION, for a source that declared `arrival: 'growing'` — `prefixOf` (one key and one digest per landed row, all that is held between readings) and `notGrowing` (the three ways a re-reading can fail the claim "it only ever appends", each its own sentence, refused `not-growing` with the rows in place untouched). Why the claim has to be checked rather than trusted: `../source/README.md`, "How the data ARRIVES" |
