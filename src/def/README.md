...
13. **A BAND IS A RANGE TOO, AND A DECLARED GESTURE THE RENDERER WILL NOT DRAW IS REFUSED AT THE DOOR.** `capabilities[i].encodings` is a declaration of a view's VOICE, and this library judges declarations: a view that says it emits an `interval` must be a view a reader can actually drag. The shape that produced this law was measured on a real page — a line chart declared `{ canProbe: true, encodings: ['interval'] }`, its x was a **band**, the chart drew **no brush element of any kind**, and a drag left every count unchanged. The page promised a drag, a caption told the reader to drag, and nothing happened. That is capability law 1 broken at the declaration.
    ...
    **AND THE LAYER THIS DOOR CANNOT REACH AT ALL: the VALUE.** A declaration carries no values, so `drawsIntervalBrush` can say *a band has no between for an interval* and still pass a renderer that draws a brush over a NUMBER and emits date-shaped strings for it — the declared KIND delivered, and nothing the column can answer. That is the same lie one layer in, and it is judged where the clause is: `intervalAddresses` / `unaddressableIntervalRefusal` (`../encoding/frame.ts`, beside this law's own predicate), asked by the renderer contract's `declared-delivered` conformance step (`vizfootprint-ui/contract/README.md`). The rule is the interval evaluator's own no-cross-type-coercion law read backwards — a quantitative axis is addressed by numeric bounds, a categorical one by strings, a temporal one by either spelling of a date — and it declines to judge anything it has no evidence about (an unfoldable type, a cleared interval, an open side):
    ...
    ```
    Today's SESSION does not refuse such a clause: it lands it, matches no row with it and files no gap — recorded as a known hole, because the honest place for that fence is the probe door, which already reads the column list it would need.
