...
The declaration has **two speakers and one owner per column**. A def states what its table's own columns are (`ColumnDecl`); an ACT states what a column it LANDS is — the same four words, `ColumnMeaning` (`../data/types.ts`) — and **the act owns a column it lands**, whole, whenever it says anything at all. That is `facets.ts` · `meaningOf`; the rule, why it is the act, and the refusal a def entry meets are in [`../analysis/README.md`](../analysis/README.md), "The `columns` channel: an act declares what it LANDS".
