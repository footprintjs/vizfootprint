...
| `layers.ts` | a layer address (`viewId~layerId`) resolved against the map — which view, which layer, which TABLE an act is gated on, whose meta it lands with. Every guard that read `this.defaultTable` reads `tableFor(address)`; a plain viewId resolves to what it always did ([`../def/README.md`](../def/README.md), "Layers") |
