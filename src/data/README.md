...
**Nothing is fetched after the bundle, in either host — how rows reach the engine.** Landing `rows` writes CSV with every column's type declared — as bytes, chunk-encoded, never one string (`landing.ts`) — and lands it through `read_csv(…, columns={…})`, and DuckDB's CSV reader is statically linked; a `csv` landing is CSV already, read through `read_csv(…, header=true, types={…})` with its types declared the same way. The browser proof (`ui/gallery/wasm.smoke.test.ts`, real Chromium) pins the number of requests that leave the origin at ZERO. It used to be one: the JSON carrier this adapter wrote made the bundle autoload its `json` extension from the vendor's extension repository, in the browser and under node alike, so an offline or CSP-restricted page could not land rows and a node landing with the repository unreachable hung. Two carriers were candidates — CSV, or JSON with the extension self-hosted — and the FIDELITY table in `engineInvariant.test.ts` decided between them: every kind of value a row can hold (null, the empty string, `0`, `-0`, NaN, `true`/`false`, a BIGINT-range integer, a 17-digit decimal, an ISO day, an ISO instant with and without milliseconds, a Date object, a comma, a quote, a newline, a CRLF, leading and trailing spaces, the null token itself, the word `NULL`, `007`, `'true'`, a string outside ASCII) landed through the shipped opener and read back through a window, `Object.is` against the memory engine. CSV agreed on every cell, and on two the JSON carrier had been LOSING: `-0` came back `0`, and an instant without milliseconds came back re-spelled with them, because the engine's sniffer had typed that column a TIMESTAMP and the millisecond spelling a VARCHAR. That is what "declared, never sniffed" buys: a column's type is tallied from the rows by the memory engine's own rule (`TypeTally`), so an ISO string is a VARCHAR on both engines and reads back byte for byte, a Date object a TIMESTAMP (read back as its ISO text), safe integers a BIGINT (a find then renders `15`, not `15.0`), any decimal a DOUBLE, and `columns()` answers the same five words from both engines for the same rows. Two cells are named exceptions on their rows, for two different reasons: a `-0` in an integer column lands as `0` because a BIGINT truly cannot carry one (no text the memory engine shows for `-0` has a sign either); NaN lands as NULL not because DuckDB has nowhere to put it — a DOUBLE holds the literal `nan` as a real, distinct value, measured — but because a stored one SORTS as DuckDB's largest number while the memory engine's own sort law reads NaN as absent, so keeping it would agree on this cell and disagree on every sorted window over the column. The null token is a bare `\N`; every string is quoted and the reader is told `allow_quoted_nulls=false`, so `""` is the empty string and `"\N"` is that text — measured on this bundle, not read off a manual. Zero rows are REFUSED with a sentence (`NO_ROWS_TO_LAND`): they name no columns, DuckDB cannot land a relation with none, and the JSON reader used to land a phantom `json` column in their place; a table that starts empty is CSV text whose header names its columns. **One type law for both kinds**, and the invariant test that forced it: a def's `csv` bytes are still the def's — DuckDB reads them as they are and detects the dialect — but their TYPES are declared from the memory engine's own sniff of the same text (`parseCSVTyped`, which never types a date out of text) through the same tally. Left to DuckDB's sniffer, a `csv` table's ISO-day column was a DATE while the rows that later refreshed it landed a VARCHAR, and the reland — which rightly reads a moved type as a change — reported `updated: 1` on a key whose bytes never changed, where the memory engine said 0. Pinned at 0 on both engines now (`engineInvariant.test.ts`, the csv-landed reland), and the same test lands a CSV text and a rows landing of the same values and asks both engines for `columns()`: one answer. A host's OWN table can still hold a real DATE or TIMESTAMP, and for it `TYPE_WORDS` and the ISO read-back exist — proven on a table made by SQL, and on a DATE-keyed host table refreshed through the port, whose removed keys come back as ISO days and whose shared key IS reported updated, because there the type really moved.

**Where the bundle comes from is the caller's to say.** `duckdbConnection({ bundles })` hands the browser arm its own map — `{ mvp: { mainModule, mainWorker }, eh: { mainModule, mainWorker } }`, the shape the package's `selectBundle` chooses over — in place of the CDN's; omitted, the CDN's map is used exactly as before, and the node arm reads the peer off disk and never consults it. The URLs are absolute, because the worker is spawned off a `blob:` URL that `importScripts` the worker script and a `blob:` URL is no base for a relative one. The gallery's wasm page is the worked example: `serve.mjs` mirrors the package's `dist/` at `/duckdb/*`, and `wasm.tsx` passes that route through the option.

```ts
const origin = globalThis.location.origin;
const open = duckdbConnection({
  bundles: {
...
**Two more arms, measured 2026-09-11** on the same bench (`bench/step0-wasm`, laws 7–10 of its README), the same instrument, the same rows — **node v22.16.0 · darwin arm64, an Apple M5 Pro with other work on the box (1-minute load 2.3 at the run's start); median of 7 / 5 / 3 repetitions at 90,300 / 300,000 / 1,000,000 rows, the spread being the max, and the p95 where n = 20**. A **moving brush** — the bench interval sliding one week per ask, 20 asks, the point clause held still so every ask keeps 3,080 matches, the one arm no cache can answer in either engine (the memory engine caches sort permutations per spec, never a clause's matches; DuckDB keeps no result cache) — costs, per ask, **3.0 / 3.4 ms** in memory / DuckDB at 90,300 rows, **9.8 / 4.9 ms** at 300,000 and **32.0 / 9.3 ms** at 1,000,000 (the p95s 4.1 / 3.6, 11.0 / 5.0, 34.0 / 9.5), and the whole 20-ask sweep as one gesture **58 / 43 ms, 193 / 74 ms, 636 / 155 ms**: a brush is twenty fresh windows, and at every size one ask of it costs what the `window` row above already says. A **wide projection** — `rows, limit 100` over the same rows carrying 30 columns (four each of six generated families beside the six above — BIGINT, DOUBLE, DATE, TIMESTAMP, VARCHAR, BOOLEAN on the run quoted here, when the JSON carrier's sniffer typed the day and instant strings; under the typed-CSV carrier those two families are VARCHAR, so DuckDB converts the BIGINT and BOOLEAN cells on the wire and copies the rest — `bench/step0-wasm/README.md`, law 8; the memory engine hands rows back by reference either way) — costs **3.81 / 5.13 ms** at 90,300 and **14.3 / 6.71 ms** at 300,000, that is **wide ÷ window = 1.26× / 1.35×** and **1.45× / 1.31×** (memory / DuckDB); at 1,000,000 the memory engine answers in **59.6 ms (1.84×)** and DuckDB in **11.2 ms (1.21×)** — a cell that WAS, under the JSON carrier, a recorded **ceiling, not a number**: the rows port serialised `{ kind: 'rows' }` as one `JSON.stringify` and V8 refuses a string past 2²⁹ − 24 characters (~512 MiB; a million 30-column rows were ~593 MiB of JSON), so DuckDB was never handed a byte (`Invalid string length`). Rows now land as typed CSV (`landing.ts`) — and as BYTES: the writer joins the CSV in chunks of whole lines (2²⁴ characters each, `LANDING_CHUNK_CHARS`) and encodes them into one `Uint8Array`, which is the only thing the engine ever took (`registerFileBuffer`, in both hosts; its `registerFileText` was one `encode` and then that), so the port has **no string ceiling at all** — the same million wide rows are 363,570,540 bytes and land, which is why the cell above is a number and not a refusal. Measured PAST the old cap on 2026-09-12 (`bench/step0-wasm`, the wide pass's two extra sizes, node v22.16.0 on the same M5 Pro, 48 GiB): **1,500,000 × 30 — ≈546 MB of CSV, over the 536,870,888-character cap — landed in 7.3 s**, and **2,000,000 × 30 — ≈728 MB — in 10.6 s**; neither the wasm heap nor the JS heap refused (`failures: []`), both engines still count the same 3,080 rows there, and a 100-row window over the 2M table answers in 18 ms (DuckDB) / 182 ms (memory). The one-string writer, run through the same bench in the same hour as a control, refused both sizes before the engine saw a byte, in V8's words: `Invalid string length`. The carrier's own cost did not move: the chunked writer and the one-string writer produce identical bytes in the same time (1M × 30: 2.54 s against 2.58 s; 1M × 6: 533 ms against 521 ms, alternating in one process) — after one wrong turn, recorded in `landing.ts` · `encodeChunks`: encoding each chunk as it was joined allocated a buffer per chunk on a heap the row loop had just dirtied, and V8 answered every allocation with a full collection (1.5 s of encode for 363 MB against 77 ms), so the chunks are encoded after the loop, into one buffer, by two passes of `encodeInto`. Two things this does NOT do. It does not move `maxMemoryRows`: the two arms rank the engines exactly as the five did — at and above 300,000 rows DuckDB wins the brush and the wide window as it wins every other read, at 90,300 the memory engine's per-ask brush is 3.0 ms and its wide window 3.81 ms, both inside an interaction budget and both bought without DuckDB's 296 ms start — so 300,000 is still the largest measured size the memory engine wins at, and a threshold that moves changes which engine `auto` picks for every dashboard, which is its own packet. And it does not touch `maxWasmRows`/`maxWasmBytes`: the landing has no ceiling of its own to carry any more, and what stops a table now — memory, the engine's heap and the page's — was not reached on this machine at 2,000,000 × 30 and is a machine fact wherever it is; `maxWasmBytes` stays `Infinity` as the host's own seam (`chooseEngine.ts` says why, and why the def door cannot fill the byte axis today). The five arms above were re-run first: 13 of 27 cells reproduced within their spread and 14 came back 3–27% slower (DuckDB's `window` at every size, the memory `sorted` and first-sort cells at 300,000 and 1,000,000 most of all) — and the 2026-09-10 code run unchanged in the same hour moved the same cells by the same amounts, so the drift is the day's machine, not the bench, and the table above keeps the quieter day's numbers with this sentence beside it.

What is still **unmeasured, and says so** rather than routing on a guess: no memory FOOTPRINT was sampled and the bench found no ceiling of DuckDB's own (it landed 1,000,000 rows of six columns and 2,000,000 rows of thirty — ≈728 MB of CSV — without complaint), so `maxMemoryBytes`, `maxWasmRows` and `maxWasmBytes` are all `Infinity` — a threshold saying "no threshold" out loud. An invented byte cap would silently veto the one number that WAS measured (the policy ANDs the axes), and an invented row ceiling would send a table to `serverProvider`, which is a typed stub that answers nothing. A host that knows its own budget passes `thresholds`. These are also NODE numbers: a browser pays a bundle download, has no `--expose-gc`, and runs the async bundle over a Worker — read the ratios, not the absolute milliseconds.
...
Three judges of "does this table have this column" used to exist and did not share what they knew. The read door asks the engine, async, on every read (`../session/session.ts` · `effectiveColumnsOf`) and narrows a clause the table cannot judge; the overview asked it again for every table on every poll and judged from that; `why()` is synchronous and could ask only the DEFINITION (`../links/reach.ts` · `columnStanding`) — so for a table declared as bare `rows`, where the definition lists no columns, `why()` credited a clause that filtered nothing as the reason for a picture it never shaped, while the Sheet and the overview both said the clause judged nothing there. The knowledge existed; it was asked for three times and kept nowhere. `landedColumns.ts` is where it is kept: `LandedColumns` — one entry per table, `{ version?, columns }`, written by the two acts that land rows (`../def/buildDashboard.ts` · `learnLanded` at each door, once per table, and the refresh door's `replaceRows` branch, which already holds the new schema in the engine's own answer) and read into the session's one reach reading (`../session/session.ts` · `tableReachAt`), where a landed list stands in for a table the definition declares no columns for — and only there: **declared columns are a promise the def door judged; landed columns are what arrived, and where the def promised nothing, what arrived is all there is.** Derived columns are laid over a landed list exactly as over a declared one, so a column an act computed is known as it always was.

It is not a cache. A cache is a guess about staleness — read through on the way to the engine, invalidated by a rule, wrong exactly when the rule is. This is written only by the acts that land rows, so it is never stale by construction, and it is never consulted by a read that must see the engine live, so it never stands in for one; it is a record of a landing, the way `sources[table].version` is a record of a fetch, and it sits beside that version because the two are facts about the same act. A provider that cannot describe its table (a stub; a wasm landing that failed) writes nothing, and a judge reading the absent entry answers as the definition alone allows — exactly the answer it gave before the registry existed (omit, never invent). The sync build door cannot await, so it SCHEDULES the learning (the memory engine answers on the next microtask, before any dispatch — an await — could commit a clause for `why()` to judge) and does not ASK a lazy wasm table AT THE DOOR: nothing has landed there yet, and asking would open the connection the door says it does not open. But a lazy landing is still a landing — the door WAITS to be told of it instead (`../def/wasmBackend.ts` · `WasmBackend.whenLanded`, a deferred that never forces the landing it reports) and learns it exactly as an eager table the moment something else — the first read, or a later `settle()` — pays for it; until then the entry is absent. Entries are copied on the way in and deep-frozen on the way out (`../detach/README.md`).

So with `sheet` declared as bare `rows` (no `columns` key) beside a `main` that carries `radius`, a brush on `main.radius` reaching the sheet's layer through the crossfilter default:

```ts
s.why({ kind: 'chart', viewId: 'table~sheet' });
// before: { ok: true, …, commits: [{ id: brush, kind: 'declaring', response: 'filter' }] } — credited, the definition said nothing
// now:    { ok: false, missing: 'declared-in-def', reached: [{ id: brush, kind: 'reaching-clause', response: 'filter',
//            narrowed: { column: 'radius', reason: 'table "sheet" has no column "radius" — a sentence about a column these rows do not have is not a claim about these rows' } }] }
// the overview's `narrowedFor` and the read door's `clauses[0].narrowed` name the same column in the same words — one knowledge, three judges
await dash.refresh(['sheet']); // the source now carries `radius`: the entry is rewritten with the new version and the mark is gone
```
...
| `landing.ts` | how a table's bytes reach DuckDB under ONE type law: rows written as CSV with every column's type tallied from the values (`TypeTally`, in DuckDB's words), a bare null token and every string quoted, read by `read_csv(…, columns={…})`; a def's CSV text registered as it came, its types declared from the memory engine's own sniff of it (`parseCSVTyped`), read by `read_csv(…, header=true, types={…})` — nothing sniffed by the engine, nothing fetched; BYTES, never one string: the CSV is joined in chunks of whole lines and encoded into one `Uint8Array` (`TypedBytes`), so the landing has no string ceiling; zero rows and a headerless text refused with the remedy named |
