...
export async function openSource(decl: SourceDecl, table: string, adapters: readonly SourceAdapter[] = []): Promise<SourceHandle> {
  ...
  if (adapter === undefined) throw new Error(`table "${table}" declares a source via ${decl.via}, and no adapter for ${decl.via} was passed — import the ${decl.via} carrier (the library's source/${decl.via} module) and pass it in \`sources\``);
